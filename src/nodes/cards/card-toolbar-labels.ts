 ...
 * with a fallback to the host card registry (CONTEXT.md: "host card").
...
export function getCardToolbarLabel(nodeType: string): string | undefined {
  return (
    CARD_DECLARATIONS.find((declaration) => declaration.nodeType === nodeType)?.toolbarLabel ??
    getHostCard(nodeType)?.toolbarLabel
  )
}
