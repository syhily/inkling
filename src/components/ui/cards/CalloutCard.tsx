...
export function CalloutCard({
  ...
  return (
    <>
      <div className={`rounded-md px-7 py-5 flex border ${CALLOUT_COLORS[color]} `} data-testid={`callout-bg-${color}`}>
        <div>
          {hasEmoji && (
            <>
              <button
                ref={emojiButtonRef}
                className={`mr-2 rounded-md px-2 text-xl cursor-pointer ${isEditing ? 'hover:bg-grey-500/20' : ''} `}
