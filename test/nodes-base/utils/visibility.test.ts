...
describe('Utils: visibility', function () {
  ...
  describe('migrateOldVisibilityFormat', function () {
    ...
    describe('web', function () {
      ...
      it(
        'uses default visibility when showOnWeb and emailOnly are not set',
        testWebMigration({}, buildDefaultVisibility().web),
      )

      it(
        'handles {emailOnly: false} as visible to all',
        testWebMigration({ emailOnly: false }, { nonMember: true, memberSegment: 'status:free,status:-free' }),
      )

      it(
        'handles {emailOnly: true} as visible to none',
        testWebMigration({ emailOnly: true }, { nonMember: false, memberSegment: '' }),
      )

      it(
        'does not use "segment" for web segments with {emailOnly: false}',
        testWebMigration(
      ...
      it(
        'handles {showOnWeb: false} as visible to none',
        testWebMigration({ showOnWeb: false }, { nonMember: false, memberSegment: '' }),
      )

      it(
        'handles {showOnWeb: true} as visible to all',
        testWebMigration({ showOnWeb: true }, { nonMember: true, memberSegment: 'status:free,status:-free' }),
      )

      it(
        'does not use "segment" for web segments with {showOnWeb: true}',
        testWebMigration(
    ...
    describe('email', function () {
      ...
      it(
        'uses default visibility if showOnEmail and emailOnly are not set',
        testEmailMigration({}, buildDefaultVisibility().email),
      )

      it(
        'handles {showOnEmail: false} as no visibility',
        testEmailMigration({ showOnEmail: false }, { memberSegment: '' }),
      )

      it(
        'handles {showOnEmail: false, segment: "status:free"} as no visibility',
        testEmailMigration({ showOnEmail: false }, { memberSegment: '' }),
      )

      it(
        'handles {showOnEmail: true, segment: ""} as visible to all',
        testEmailMigration({ showOnEmail: true }, { memberSegment: 'status:free,status:-free' }),
      )

      it(
        'handles {showOnEmail: true, segment: "status:free"} as visible to free',
        testEmailMigration({ showOnEmail: true, segment: 'status:free' }, { memberSegment: 'status:free' }),
      )

      it(
        'handles {showOnEmail: true, segment: "status:paid"} as visible to paid',
        testEmailMigration({ showOnEmail: true, segment: 'status:paid' }, { memberSegment: 'status:-free' }),
      )

      it(
        'handles {showOnEmail: true, segment: "status:-free+status:-paid"} as no visible',
        testEmailMigration({ showOnEmail: true, segment: 'status:-free+status:-paid' }, { memberSegment: '' }),
      )
