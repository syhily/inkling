...
- Only Bookmark, Header, and Toggle keep hand-written wrapper nodes (`src/nodes/*Node.tsx`) extending base nodes (`src/nodes/base/nodes/*/`). The other cards are collapsed to their declarations (`src/nodes/cards/*.declaration.ts`): their registered class is assembled by `src/nodes/assemble-card-node.ts`, and the old wrapper paths remain as transition shims re-exporting the assembled class and the base-canonical `$is*`. Base node classes are named `Base*Node` (e.g. `BaseAudioNode`) with a `$createBase*Node` factory; the public `*Node` name / `$create*Node` factory belong to the wrapper or shim and construct the assembled class. Each card declaration also carries its insert command, card-menu entry, and drag icon — `src/nodes/cards/card-commands.ts`, the menu/drag-icon registries, and the `src/nodes/cards/decorate/*.tsx` targets are derived views over the declarations, so adding a card means writing a declaration plus one decorate module.
