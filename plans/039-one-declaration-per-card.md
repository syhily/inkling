...
Plan 039 landed in five batches on main (`6fc2399..0f975a6`). Batch 1 (`6fc2399` + `cada902` comment fix) added 13 React-free card declarations and derived every node-set registry (DefaultNodes, EmailNodes, EmailEditorNodes, base DEFAULT_NODES, DEFAULT_HTML_NODES) behind a diff test pinning the pre-refactor arrays — which also severed a pre-existing latent `@/index` cycle (InklingCaptionEditor/InklingComposableEditor/InklingNestedEditor barrel edges) that could put silent `undefined` entries into registries depending on entry order. Batch 2 (`7fe7c7f` + `4403317`) derived MARKDOWN_NODES and CARD_TRANSFORMERS with the legacy transformer order pinned. Batch 3 (`056e63d`) absorbed the 8 nested-editor trilogies into the generator's `nestedEditors` spec, preserving the per-card cleanBasicHtml matrix and Header's hidden InitialState keys. Batch 4 (`1650603`) absorbed cardMenu (arrays normalized), transient upload props, and the single `decorate-card.tsx` adapter; `getIcon()` ×13 deleted. Batch 5 (`4512bef`) collapsed ten cards to declarations via `assembleCardNode`, leaving Bookmark/Header/Toggle as hand-written wrappers and the old wrapper paths as shims re-exporting the assembled class and base-canonical `$is*`; `988e7e7` applied post-review fixes (ZWNJNode own importJSON, optional cardMenu type, dead className prop drop, triage retarget) and `0f975a6` fixed the review's format/citation nits. Accepted deviations: declarations are React-free with wrapper-layer projections (card-wrappers, card-menus, card-markdown-transformers, card-decorate) holding the React-bearing knowledge after the TDZ cycle materialized; `LEGACY_DEFAULT_CARD_ORDER`/`MARKDOWN_CARD_ORDER` pins reproduce the two historically divergent orders; `$create*` stayed per-shim; buildCardMenu's bare-object tolerance kept as one coercion line (pinned by fixture tests). Gates at HEAD: typecheck clean, lint 0/0, format:check clean (819 files), unit 205 files / 1666 passed / 21 todo; card e2e 212/212 and `verify:package` PASS at `4512bef`. Follow-ups: optionally convert Bookmark's `__createdWithUrl` to a transient-spec entry; the 038 test-plan row "handler reads fresh value without re-render" has no dedicated unit test (accepted gap — structurally guaranteed and e2e-covered); AGENTS.md could name the four projection modules for discoverability.
