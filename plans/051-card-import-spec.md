...
| ---- | ------ | -------------------------- |
| bookmark | `bookmark/bookmark-parser.ts` | nested `metadata` object payload with the reversed author/publisher classes quirk (:14-15); the `mixtapeEmbed` conversion mutates the DOM (`titleElement.remove()` :60, `descElement.remove()` :66) and derives publisher from the *remaining* innerHTML (:70) |
...
- A base-node constructor override coalesces on key *presence* rather than
